...
def comprimir_audio(archivo_original, archivo_comprimido):
    ...
    if not archivo_original.lower().endswith('.mp3') and not archivo_original.lower().endswith('.gz'):
        raise ErrorExtension("El archivo no tiene la extensión .mp3 o .gz")
...
def comprimir_audio_tamaño_corto(archivo_original, archivo_comprimido):
...
def comprimir_audio_tamaño_largo(archivo_original, archivo_comprimido):
    file_size_kb = os.path.getsize(archivo_original) / 1024
...
def descomprimir_audio_tamaño_corto(archivo_comprimido, archivo_descomprimido):
...
def descomprimir_audio_tamaño_largo(archivo_comprimido, archivo_descomprimido):
    tamaño_audio = os.path.getsize(archivo_comprimido) / 1024

    if tamaño_audio  >= tamaño_umbral_archivo:
        print("El archivo de audio comprimido es largo.")
        descomprimir_audio(archivo_comprimido, archivo_descomprimido)